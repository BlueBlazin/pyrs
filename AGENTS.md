...
- Full stdlib baseline checkpoint (2026-02-25):
  ...
  - latest stdlib closure deltas:
    ...
    - `_threading_local` post-import fixup now rebinds `_thread._local` to the CPython pure-Python implementation (`_threading_local.local`) for semantic parity over placeholder native class behavior,
    ...
    - CLI startup now sets CPython-shaped `sys.argv` across modes:
      - script mode: `[script_path, ...script_args]`,
      - `-c` mode: `['-c', ...args]`,
      - REPL/stdin mode: `['']`,
      closing subprocess script-argv drift that previously broke `pygettext` option parsing,
    - `pwd` native substrate now implements `getpwall`, `getpwnam`, and `getpwuid` from `/etc/passwd` with tuple-backed `struct_passwd` instances; targeted `test.test_pwd` lane is green,
    - `bytes.split` / `bytearray.split` now support separator + maxsplit + whitespace semantics with receiver-typed chunk output, unblocking `test.test_nturl2path`,
    - `unicodedata` now exports baseline `category`/`bidirectional` on both `unicodedata` and `unicodedata.ucd_3_2_0`; remaining surrogate-codepoint fidelity gap is tracked in `docs/STUB_ACCOUNTING.md`,
...
- Source-language parity checkpoint (2026-02-23, latest):
  ...
  - Language feature gate now includes:
    ...
    plus chapter/kind umbrella probes and grammar section probes for full
    inventory mapping; current manifest run is `29/29` probes passing.
