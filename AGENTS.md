...
- `test.test_io` failfast probe now clears CIOTest + PyIOTest and reaches deeper CBufferedReader coverage; current first blocker is `CBufferedReaderTest.test_close_error_on_close` (`close` vs `flush` exception-context ordering).
