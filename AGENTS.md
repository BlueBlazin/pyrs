...
- Newly landed parity checkpoints:
  ...
  - startup/import checkpoint: import resolver state now tracks `sys.path`/`meta_path`/`path_hooks` signatures to avoid repeated default-finder scans, default CPython stdlib auto-detection now selects a single canonical fallback root (instead of loading every discovered system path), and source-bound pyc fallback diagnostics now report exact translate/load failure reasons when `PYRS_IMPORT_PERF_VERBOSE=1`.
  - pyc translator checkpoint: marshal reader/writer now supports set/frozenset constants (`<`/`>`), `BINARY_OP` mapping covers the full CPython 3.14 arg table (including bitwise/shift/matmul and inplace variants), and f-string translation paths now handle `CONVERT_VALUE`/`FORMAT_SIMPLE`/`FORMAT_WITH_SPEC` plus `BUILD_STRING`.
  - startup pyc-preference note: with `PYRS_IMPORT_PREFER_PYC=1`, the current remaining blockers in startup imports are additional CPython opcode-translation gaps (`DICT_MERGE`, `COPY`, and high-bit `COMPARE_OP` args such as `72`), which are now the next import-startup closure target.
