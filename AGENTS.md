...
- Extension scaffolding checkpoint:
  ...
  - current direct NumPy import blocker is post-init semantic closure in pure-Python core bootstrap: `numpy._core.numerictypes` is incomplete, so `numpy._core.numeric` does not publish `False_`/`True_`, and top-level `import numpy` fails with `ImportError: cannot import name 'False_' from 'numpy._core'`.
  ...
  - immediate NumPy priority is deterministic closure of `_core.numerictypes` module-state/attr publication semantics (no trial-and-error attr patching), then re-run direct gate (`perf/numpy_gate_direct_latest.json`).
