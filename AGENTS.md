...
- Parser: packrat-style memoization with a minimal lexer, indentation tokens, `if`/`elif`/`else`/`while`/`for`, `break`/`continue`, `with`, function defs, returns, calls, `global`/`nonlocal`, and tuple/list destructuring targets.
