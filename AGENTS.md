...
- Newly landed parity checkpoints:
  ...
  - startup pyc-preference note: with `PYRS_IMPORT_PREFER_PYC=1`, `import site` now succeeds and current fallbacks are explicitly due to unsupported CPython exception tables (e.g. `site`, `_sitebuiltins`, `stat`), not the earlier opcode gaps.
