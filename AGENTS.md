...
7. Milestone 9 is in progress: float runtime/parsing semantics and `random` module foundations are implemented; remaining Milestone 9 scope is runtime type parity (`set`/`bytes` families, unicode/codecs), metaclass/`__slots__` parity, and broader stdlib bootstrap.
8. Remaining milestones 10-14 cover: full async/concurrency semantics, CPython parity test gates, performance/observability, packaging/distribution, and future JIT/extension hooks.
...
- Runtime data model (Milestone 8): descriptor-aware attribute load/store paths (`__get__`, `__set__`, `__delete__`), instance hooks (`__getattr__`, `__setattr__`, `__delattr__`), C3 MRO metadata (`__mro__`, `__bases__`), explicit-args `super(type, obj)` lookup object, and builtin attr helpers (`getattr`, `setattr`, `delattr`, `hasattr`).
...
- Classes: class bodies execute in a class namespace module while resolving missing names against the defining module; methods capture the defining module as globals; C3 MRO is computed at class creation and exposed via `__mro__`.
...
- Stdlib bootstrap additions: builtin `random` module is preinstalled (`seed`, `random`, `randrange`, `randint`, `getrandbits`, `choice`, `shuffle`) with deterministic reseed behavior.
