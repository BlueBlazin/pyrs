...
- Extension scaffolding checkpoint:
  ...
  - current NumPy direct-mode blocker is now semantic/type-identity closure during `_multiarray_umath` `Py_mod_exec` (first-failure message currently reports dtype-instance validation mismatch after deep init progress), not unresolved symbols.
