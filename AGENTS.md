...
- Newly landed parity checkpoints:
  ...
  - REPL checkpoint: no-arg CLI path now starts an interactive `reedline` REPL (`RSPYTHON` banner, history, tab completion, multiline, `:paste`/`:timing`/`:reset` controls, expression-result echo), while non-interactive no-arg runs consume stdin as script input (CPython-like `python < file.py` behavior).
