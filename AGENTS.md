...
- Newly landed parity checkpoints:
  ...
  - startup/import checkpoint: positive module-source lookup cache (`(root, module_name)`) and `sys.path` sync short-circuit/invalidation are landed; startup pass-site improved from `0.0275s` to `0.0225s` user in local `scripts/bench_startup_gate.sh` runs (12 iterations, warmup 2).
