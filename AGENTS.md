...
- Extension scaffolding checkpoint:
  ...
  - current direct NumPy import blocker is in `_multiarray_umath` init (`get_wrapped_legacy_ufunc_loop`): caller context (`PyArrayMethod_Context.caller`) is null during legacy wrapping loop bring-up (`cannot call %s without its ufunc as caller context`).
