...
- Newly landed parity checkpoints:
  ...
  - perf checkpoint: `BinaryAdd -> ReturnValue` now fuses for simple one-arg/no-cells frames, and release builds use `lto = "fat"`; local gate currently measures `fib(29)` at ~`0.13s` user (`scripts/bench_fib_gate.sh`).
