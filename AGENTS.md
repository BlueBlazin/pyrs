...
- Extension scaffolding checkpoint:
  ...
  - ABI coverage baseline is generated from CPython 3.14 `Misc/stable_abi.toml` via `scripts/generate_abi3_manifest.py`; current snapshot is `functions 467/782`, `data 50/143` in `perf/abi3_manifest_latest.json`.
  ...
  - additional abi3 closure slices landed: `PyCallIter_New` (with callable/sentinel iterator substrate), `PyGILState_GetThisThreadState` (with non-null/stable thread-state baseline), and `PyEval_{GetGlobals,GetLocals}` (frame mapping accessors with no-frame null-return baseline).
