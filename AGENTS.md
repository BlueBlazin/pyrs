...
- Runtime data model (Milestones 8-9): descriptor-aware attribute load/store paths (`__get__`, `__set__`, `__delete__`), instance hooks (`__getattr__`, `__setattr__`, `__delattr__`), C3 MRO metadata (`__mro__`, `__bases__`), explicit-args `super(type, obj)` lookup object, class-header `metaclass=` path, `__build_class__` metaclass kwarg handling, `__slots__` restrictions, and builtin attr helpers (`getattr`, `setattr`, `delattr`, `hasattr`).
