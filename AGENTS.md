...
- Newly landed parity checkpoints:
  ...
  - `_sqlite3.set_trace_callback()` now records VM-executed statement text for connection paths (including implicit transactional statements generated by sqlite connection helpers).
  - `sqlite3` DB-API failfast probe (`Lib/test/test_sqlite3/test_dbapi.py`) is now green locally.
  - strict stdlib harness suite now includes `test/test_sqlite3/test_dbapi.py`, `test/test_sqlite3/test_dump.py`, and `test/test_sqlite3/test_transactions.py` and stays green (`PYRS_RUN_STRICT_STDLIB=1 cargo test -q --test cpython_harness runs_cpython_strict_stdlib_suite`).
  ...
  - `_sqlite3` known long-tail (tracked): `test.test_sqlite3.test_factory` still has row-specific gaps (`dict(Row)` mapping conversion and ordering-operator TypeError parity), so strict suite coverage remains `test_dbapi`/`test_dump`/`test_transactions` for now.
