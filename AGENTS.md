...
- Extension scaffolding checkpoint:
  ...
  - C-API v1 now includes capsule baseline helpers (`capsule_new`, `capsule_get_pointer`, `capsule_get_name`, `capsule_set_context`, `capsule_get_context`, `capsule_set_destructor`) for opaque native-pointer interop in extension handle space.
