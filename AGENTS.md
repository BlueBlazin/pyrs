...
- Exceptions: `try/except` handles explicit `raise`; VM runtime errors are mapped to coarse exception types (`RuntimeError`, `TypeError`, `IndexError`, `KeyError`, `ZeroDivisionError`, `NameError`, `AttributeError`) based on message heuristics; tracebacks include filename/line/column and frame names; exception chaining metadata is wired for `raise ... from ...` and implicit context (`__cause__`, `__context__`, `__suppress_context__`).
