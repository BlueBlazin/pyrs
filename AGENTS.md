...
- Extension scaffolding checkpoint:
  ...
  - current direct NumPy import blocker is pure-Python module completion/publication integrity in `numpy.lib` import chains: top-level `import numpy` currently fails with `ImportError: cannot import name 'pad' from 'numpy.lib._arraypad_impl'` (direct gate artifact: `perf/numpy_gate_direct_latest.json`).
  ...
  - C-side varargs handling now also covers `PyArg_ParseTuple`, `PyArg_ParseTupleAndKeywords`, `PyObject_CallFunctionObjArgs`, and `PyObject_CallMethod` in `src/vm/capi_variadics.c`.
  ...
  - immediate NumPy priority is deterministic closure of `numpy.lib` module-state/attr publication semantics (no per-attribute patching), then re-run direct gate (`perf/numpy_gate_direct_latest.json`).
...
- Newly landed parity checkpoints:
  ...
  - stack-heavy pure-stdlib tests now also pin dedicated 32MB stacks where needed (`tests/vm.rs::json_import_prefers_cpython_pure_module_when_lib_path_is_added_by_default`, `tests/vm.rs::statistics_mean_supports_basic_int_dataset`, `tests/hashlib_native.rs::hashlib_module_uses_native_md5_and_sha256_backends`).
