...
- Extension scaffolding checkpoint:
  ...
  - current direct NumPy import blocker is in `_multiarray_umath` init after legacy-loop caller-context bring-up: native paths now fail later with bytes/complex argument-shape mismatch (`PyBytes_AsString expected bytes object`, with observed non-bytes inputs like `float`/`bool` during loop-registration bring-up).
