...
- Extension scaffolding checkpoint:
  ...
  - C-API handle constructors/getters now include `None`/`float`/`bytes`/`tuple`/`list` in addition to `bool`/`int`/`str`, with smoke coverage through native handle and sequence round-trip.
