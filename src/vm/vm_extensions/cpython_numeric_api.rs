...
fn cpython_type_name(type_ptr: *mut CpythonTypeObject) -> String {
    ...
    // SAFETY: caller provides a valid type pointer from an inspected object header.
...
pub unsafe extern "C" fn PyNumber_Index(object: *mut c_void) -> *mut c_void {
    with_active_cpython_context_mut(|context| {
        if object.is_null() {
            context.set_error("PyNumber_Index expected object");
            return std::ptr::null_mut();
        }
        ...
        if let Some(value) = mapped_value.clone() {
            if context.vm.is_null() {
                context.set_error("PyNumber_Index missing VM context");
                return std::ptr::null_mut();
            }
            // SAFETY: VM pointer is valid for the active context lifetime.
            let vm = unsafe { &mut *context.vm };
            match vm.capi_coerce_index_value(value.clone()) {
                Ok(exact) => return context.alloc_cpython_ptr_for_value(exact),
                Err(err) => {
                    if let Some(proxy_raw_ptr) =
                        ModuleCapiContext::cpython_proxy_raw_ptr_from_value(&value)
                        && !proxy_raw_ptr.is_null()
                    {
                        slot_object = proxy_raw_ptr;
                    } else {
        ...
        // SAFETY: `slot_object` points at a CPython object when present.
        let type_ptr = unsafe {
            slot_object
                .cast::<CpythonObjectHead>()
                .as_ref()
        ...
        // SAFETY: `type_ptr` is non-null and points to a valid type object.
        let number_methods = unsafe {
            (*type_ptr)
                .tp_as_number
                .cast::<CpythonNumberMethods>()
        ...
        let Some(number_methods) = number_methods else {
            cpython_set_typed_error(
                ...
                format!(
                    ...
                    cpython_type_name(type_ptr)
                ),
            );
            return std::ptr::null_mut();
        };
        let Some(converter) = number_methods.nb_index else {
            cpython_set_typed_error(
                unsafe { PyExc_TypeError },
                format!(
                    "'{}' object cannot be interpreted as an integer",
        ...
        // SAFETY: `converter` is the nb_index slot for `slot_object`.
        let result = unsafe { converter(slot_object) };
        if result.is_null() {
            if unsafe { PyErr_Occurred() }.is_null() {
                context.set_error("PyNumber_Index returned NULL without setting an exception");
            }
            return std::ptr::null_mut();
        }

        // SAFETY: `result` is a non-null Python object pointer.
        ...
        let is_long = !result_type.is_null()
            && (result_type == std::ptr::addr_of_mut!(PyLong_Type)
                // SAFETY: both pointers are valid type objects.
                || unsafe {
                    PyType_IsSubtype(
                        result_type.cast::<c_void>(),
                        std::ptr::addr_of_mut!(PyLong_Type).cast::<c_void>(),
                    ) != 0
                });
        if !is_long {
            let type_name = cpython_type_name(result_type);
        ...
        if result_type == std::ptr::addr_of_mut!(PyLong_Type) {
            return result;
        }

        let exact = unsafe { PyNumber_Long(result) };
        unsafe { Py_DecRef(result) };
        exact
    })
    .unwrap_or_else(|err| {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn PyNumber_AsSsize_t(object: *mut c_void, exc: *mut c_void) -> isize {
    match with_active_cpython_context_mut(|context| {
        if object.is_null() {
            context.set_error("PyNumber_AsSsize_t expected object");
            return -1;
        }
        ...
        if let Some(value) = context.cpython_value_from_ptr(object) {
            if context.vm.is_null() {
                context.set_error("PyNumber_AsSsize_t missing VM context");
                return -1;
            }
            // SAFETY: VM pointer is valid for the active context lifetime.
            let vm = unsafe { &mut *context.vm };
            let type_name = vm.value_type_name_for_error(&value);
            let indexed = match vm.capi_coerce_index_value(value.clone()) {
                Ok(value) => value,
                Err(err) => {
                    if let Some(proxy_raw_ptr) =
                        ModuleCapiContext::cpython_proxy_raw_ptr_from_value(&value)
            ...
            if !matches!(indexed, Value::None) {
                return match vm.capi_long_as_ssize_t(indexed.clone()) {
                    Ok(value) => value,
                    Err(err) if err.message == "Python int too large to convert to C ssize_t" => {
                        if exc.is_null() {
                            let is_negative = match indexed {
        ...
        let index_value = unsafe { PyNumber_Index(slot_object) };
        if index_value.is_null() {
            return -1;
        }
        let result = unsafe { PyLong_AsSsize_t(index_value) };
        ...
        if !exc.is_null() {
            unsafe { PyErr_Clear() };
            // SAFETY: `index_value` is a valid Python object.
