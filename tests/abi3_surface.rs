...
#[test]
fn exports_abi3_batch42_symbols() {
    let symbols = exported_symbols(&pyrs_bin());
    let required = [
        "PyThreadState_New",
        "PyThreadState_Swap",
    ...
    assert!(
        missing.is_empty(),
        "missing ABI batch42 symbols: {missing:?}"
    );
}
...
#[test]
fn exports_abi3_batch47_symbols() {
    let symbols = exported_symbols(&pyrs_bin());
    let required = [
        "PyState_AddModule",
        "PyState_FindModule",
    ...
    assert!(
        missing.is_empty(),
        "missing ABI batch47 symbols: {missing:?}"
    );
}

#[test]
fn exports_abi3_batch48_symbols() {
    let symbols = exported_symbols(&pyrs_bin());
    let required = ["PyMember_GetOne", "PyMember_SetOne"];
    let missing: Vec<&str> = required
        .iter()
        .copied()
        .filter(|name| !symbols.contains(*name))
    ...
    assert!(
        missing.is_empty(),
        "missing ABI batch48 symbols: {missing:?}"
    );
}
...
#[test]
fn exports_abi3_batch55_symbols() {
    let symbols = exported_symbols(&pyrs_bin());
    let required = ["PyTraceBack_Here", "PyTraceBack_Print"];
    let missing: Vec<&str> = required
        .iter()
        .copied()
        .filter(|name| !symbols.contains(*name))
    ...
    assert!(
        missing.is_empty(),
        "missing ABI batch55 symbols: {missing:?}"
    );
}
...
#[test]
fn exports_abi3_batch58_symbols() {
    let symbols = exported_symbols(&pyrs_bin());
    let required = [
    ...
    assert!(
        missing.is_empty(),
        "missing ABI batch58 symbols: {missing:?}"
    );
}
...
#[test]
fn exports_abi3_batch63_symbols() {
    let symbols = exported_symbols(&pyrs_bin());
    let required = ["PyUnicode_FromFormat", "PyUnicode_FromFormatV"];
    let missing: Vec<&str> = required
        .iter()
        .copied()
        .filter(|name| !symbols.contains(*name))
    ...
    assert!(
        missing.is_empty(),
        "missing ABI batch63 symbols: {missing:?}"
    );
}
...
#[test]
fn exports_abi3_batch67_symbols() {
    let symbols = exported_symbols(&pyrs_bin());
    let required = [
        "Py_EndInterpreter",
        "Py_Finalize",
    ...
    assert!(
        missing.is_empty(),
        "missing ABI batch67 symbols: {missing:?}"
    );
}
...
#[test]
fn exports_abi3_batch69_symbols() {
    let symbols = exported_symbols(&pyrs_bin());
    let required = [
    ...
    assert!(
        missing.is_empty(),
        "missing ABI batch69 symbols: {missing:?}"
    );
}
