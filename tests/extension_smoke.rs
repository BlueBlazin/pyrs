...
#[test]
fn cpython_compat_eval_frame_abi_batch41_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch41 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch41 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch41");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch41_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
...
run(PyObject *self, PyObject *args) {
    ...
    int frame_present = PyEval_GetFrame() != 0 ? 1 : 0;

    PyObject *value1 = PyEval_EvalFrame(0);
    int eval1_ok = (value1 == 0 && PyErr_Occurred() != 0) ? 1 : 0;
    PyErr_Clear();

    PyObject *value2 = PyEval_EvalFrameEx(0, 0);
    int eval2_ok = (value2 == 0 && PyErr_Occurred() != 0) ? 1 : 0;
    PyErr_Clear();

    return Py_BuildValue("(iii)", frame_present, eval1_ok, eval2_ok);
}

static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe PyEval_EvalFrame APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch41_probe",
    "cpython api batch41 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch41_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch41 extension should build");

    run_import_snippet(
        &bin,
        &temp_root,
        "import cpython_api_batch41_probe as m\nres = m.run()\nassert res == (1, 1, 1)",
    )
    .expect("cpython api batch41 extension import should succeed");
...
#[test]
fn cpython_compat_threadstate_abi_batch42_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch42 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch42 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch42");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch42_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
...
run(PyObject *self, PyObject *args) {
    ...
    void *orig = PyThreadState_Get();
    void *interp = PyInterpreterState_Get();
    void *created = PyThreadState_New(interp);
    int created_ok = created != 0 ? 1 : 0;

    void *prev = created ? PyThreadState_Swap(created) : 0;
    int swap_to_created_ok = (created && prev == orig) ? 1 : 0;
    int now_created_ok = (created && PyThreadState_Get() == created) ? 1 : 0;

    void *back = orig ? PyThreadState_Swap(orig) : 0;
    int swap_back_ok = (created && back == created) ? 1 : 0;

    unsigned long long tid = PyThreadState_GetID(orig);
    int async_hit = PyThreadState_SetAsyncExc(tid, (PyObject *)PyExc_RuntimeError);
    int async_miss = PyThreadState_SetAsyncExc(tid + 999999ULL, (PyObject *)PyExc_RuntimeError);

    if (created) {
        PyThreadState_Clear(created);
        PyThreadState_Delete(created);
    }
...
static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe PyThreadState lifecycle APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch42_probe",
    "cpython api batch42 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch42_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch42 extension should build");

    run_import_snippet(
        &bin,
        &temp_root,
        "import cpython_api_batch42_probe as m\nres = m.run()\nassert res == (1, 1, 1, 1, 1, 1)",
    )
    .expect("cpython api batch42 extension import should succeed");
...
#[test]
fn cpython_compat_sys_abi_batch44_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch44 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch44 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch44");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch44_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
#include <wchar.h>
...
run(PyObject *self, PyObject *args) {
    ...
    wchar_t *argv_items[2];
    argv_items[0] = L"/tmp/prog.py";
    argv_items[1] = L"arg1";
    PySys_SetArgvEx(2, argv_items, 1);
    PySys_SetArgv(2, argv_items);

#ifdef _WIN32
    PySys_SetPath(L"one;two");
#else
    PySys_SetPath(L"one:two");
#endif

    PyObject *argv_obj = PySys_GetObject("argv");
    PyObject *path_obj = PySys_GetObject("path");
    int argv_ok = (argv_obj && PySequence_Size(argv_obj) == 2) ? 1 : 0;
    int path_ok = (path_obj && PySequence_Size(path_obj) >= 2) ? 1 : 0;
    ...
    PySys_WriteStdout("batch44 stdout %d\n", 7);
    PySys_WriteStderr("batch44 stderr %s\n", "ok");
...
static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe extended sys APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch44_probe",
    "cpython api batch44 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch44_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch44 extension should build");
...
#[test]
fn cpython_compat_pystate_abi_batch47_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch47 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch47 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch47");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch47_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
...
run(PyObject *self, PyObject *args) {
    ...
    PyObject *before = PyState_FindModule(&module_def);
    int before_null = before == 0 ? 1 : 0;
    int add_ok = PyState_AddModule(self, &module_def) == 0 ? 1 : 0;
    PyObject *found = PyState_FindModule(&module_def);
    int found_ok = (found == self || found != 0) ? 1 : 0;
    int remove_ok = PyState_RemoveModule(&module_def) == 0 ? 1 : 0;
    PyObject *after = PyState_FindModule(&module_def);
    int after_null = after == 0 ? 1 : 0;

    return Py_BuildValue("(iiiii)", before_null, add_ok, found_ok, remove_ok, after_null);
}

static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe pystate APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch47_probe",
    "cpython api batch47 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch47_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch47 extension should build");

    run_import_snippet(
        &bin,
        &temp_root,
        "import cpython_api_batch47_probe as m\nres = m.run()\nassert res[0] in (0, 1)\nassert res[1:] == (1, 1, 1, 1)",
    )
    .expect("cpython api batch47 extension import should succeed");
...
#[test]
fn cpython_compat_structseq_abi_batch49_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch49 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch49 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch49");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch49_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
...
run(PyObject *self, PyObject *args) {
    ...
    PyTypeObject *tp = PyStructSequence_NewType(&desc);
    int type_ok = tp != 0 ? 1 : 0;
    PyObject *obj = tp ? PyStructSequence_New(tp) : 0;
    int new_ok = obj != 0 ? 1 : 0;

    PyObject *left = PyLong_FromLong(3);
    PyObject *right = PyLong_FromLong(4);
    if (obj && left && right) {
        PyStructSequence_SetItem(obj, 0, left);
        PyStructSequence_SetItem(obj, 1, right);
    }
    else {
        Py_XDECREF(left);
        Py_XDECREF(right);
    ...
    PyObject *first = obj ? PyStructSequence_GetItem(obj, 0) : 0;
    PyObject *second = obj ? PyStructSequence_GetItem(obj, 1) : 0;
    int get_ok = (first && second &&
                  PyLong_AsLong(first) == 3 &&
                  PyLong_AsLong(second) == 4) ? 1 : 0;

    int unnamed_ok = (PyStructSequence_UnnamedField != 0 &&
                      PyStructSequence_UnnamedField[0] != '\0') ? 1 : 0;

    Py_XDECREF(obj);
    return Py_BuildValue("(iiii)", type_ok, new_ok, get_ok, unnamed_ok);
}

static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe structseq ABI APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch49_probe",
    "cpython api batch49 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch49_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch49 extension should build");

    run_import_snippet(
        &bin,
        &temp_root,
        "import cpython_api_batch49_probe as m\nres = m.run()\nassert res == (1, 1, 1, 1)",
    )
    .expect("cpython api batch49 extension import should succeed");
...
#[test]
fn cpython_compat_exceptions_abi_batch51_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch51 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch51 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch51");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch51_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
...
run(PyObject *self, PyObject *args) {
    ...
    int nonnull_ok = (PyExc_BaseException &&
                      PyExc_ArithmeticError &&
                      PyExc_EncodingWarning &&
                      PyExc_StopIteration &&
                      PyExc_ModuleNotFoundError) ? 1 : 0;

    int alias_ok = (PyExc_EnvironmentError == PyExc_OSError &&
                    PyExc_IOError == PyExc_OSError &&
                    PyExc_WindowsError == PyExc_OSError) ? 1 : 0;

    PyErr_SetString(PyExc_ZeroDivisionError, "boom");
    int zdiv_ok = (PyErr_Occurred() == PyExc_ZeroDivisionError) ? 1 : 0;
    PyErr_Clear();
    ...
    int bytes_warn_ok = (PyErr_Occurred() == PyExc_BytesWarning) ? 1 : 0;
    PyErr_Clear();
...
static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe exception ABI APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch51_probe",
    "cpython api batch51 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch51_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch51 extension should build");
...
#[test]
fn cpython_compat_traceback_abi_batch55_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch55 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch55 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch55");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch55_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
...
run(PyObject *self, PyObject *args) {
    (void)args;
    PyObject *stderr_obj = PySys_GetObject("stderr");
    if (!stderr_obj) {
        return 0;
    }

    PyObject *tb = PyUnicode_FromString("traceback-probe");
    if (!tb) {
        return 0;
    }

    int print_ok = (PyTraceBack_Print(tb, stderr_obj) == 0) ? 1 : 0;
    int here_ok = (PyTraceBack_Here(self) == 0) ? 1 : 0;
    int null_guard_ok = (PyTraceBack_Here(0) == -1) ? 1 : 0;
    PyErr_Clear();

    Py_DECREF(tb);

    return Py_BuildValue("(iii)", print_ok, here_ok, null_guard_ok);
}

static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe traceback ABI APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch55_probe",
    "cpython api batch55 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch55_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch55 extension should build");

    run_import_snippet(
        &bin,
        &temp_root,
        "import cpython_api_batch55_probe as m\nres = m.run()\nassert res == (1, 1, 1)",
    )
    .expect("cpython api batch55 extension import should succeed");
...
#[test]
fn cpython_compat_unicode_append_compare_abi_batch59_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch59 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch59 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch59");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch59_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
#include <string.h>
...
run(PyObject *self, PyObject *args) {
    ...
    Py_DECREF(right);
    int append_ok = utf8_equals(left, "abcd");

    PyObject *tail = PyUnicode_FromString("ef");
    PyUnicode_AppendAndDel(&left, tail);
    int append_del_ok = utf8_equals(left, "abcdef");

    PyObject *interned = PyUnicode_FromString("token");
    PyUnicode_InternInPlace(&interned);
    int intern_in_place_ok = utf8_equals(interned, "token");
    PyUnicode_InternImmortal(&interned);
    int intern_immortal_ok = utf8_equals(interned, "token");

    long long size = PyUnicode_GetSize(interned);
    int get_size_removed_ok = (size == -1 && PyErr_Occurred() != 0) ? 1 : 0;
    PyErr_Clear();

    PyObject *a = PyUnicode_FromString("a");
    PyObject *b = PyUnicode_FromString("b");
    PyObject *eq = PyUnicode_RichCompare(a, a, 2);
    PyObject *lt = PyUnicode_RichCompare(a, b, 0);
    PyObject *ne = PyUnicode_RichCompare(a, b, 3);
    int rich_compare_ok = (eq && lt && ne &&
                           PyObject_IsTrue(eq) == 1 &&
                           PyObject_IsTrue(lt) == 1 &&
                           PyObject_IsTrue(ne) == 1) ? 1 : 0;
    Py_XDECREF(ne);
    Py_XDECREF(lt);
    Py_XDECREF(eq);
    ...
    PyObject *two = PyLong_FromLong(2);
    PyObject *not_impl = PyUnicode_RichCompare(one, two, 2);
    int rich_notimpl_ok = (not_impl != 0) ? 1 : 0;
    Py_XDECREF(not_impl);
    Py_XDECREF(two);
    Py_XDECREF(one);
...
static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe unicode append/compare ABI APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch59_probe",
    "cpython api batch59 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch59_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch59 extension should build");

    run_import_snippet(
        &bin,
        &temp_root,
        "import cpython_api_batch59_probe as m\nres = m.run()\nassert res == (1, 1, 1, 1, 1, 1, 1), res",
    )
    .expect("cpython api batch59 extension import should succeed");
...
fn cpython_compat_unicode_fromformat_abi_batch63_apis_work() {
    ...
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch63 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch63");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch63_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
#include <stdarg.h>
...
call_fromformatv(const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    PyObject *result = PyUnicode_FromFormatV(fmt, ap);
    va_end(ap);
    return result;
}
...
static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe unicode fromformat ABI APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch63_probe",
    "cpython api batch63 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch63_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch63 extension should build");
...
#[test]
fn cpython_compat_pathconfig_and_locale_abi_batch66_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch66 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch66 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch66");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch66_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
#include <string.h>
...
run(PyObject *self, PyObject *args) {
    ...
    wchar_t program_name[] = L"/tmp/pyrs66_program";
    wchar_t python_home[] = L"/tmp/pyrs66_home";
#ifdef _WIN32
    wchar_t module_path[] = L"/tmp/pyrs66_a;/tmp/pyrs66_b";
#else
    wchar_t module_path[] = L"/tmp/pyrs66_a:/tmp/pyrs66_b";
#endif
    ...
    int program_ok = (wcscmp(Py_GetProgramName(), program_name) == 0) ? 1 : 0;
    int home_ok = (wcscmp(Py_GetPythonHome(), python_home) == 0) ? 1 : 0;
    int path_ok = (wcscmp(Py_GetPath(), module_path) == 0) ? 1 : 0;

    wchar_t **argv_out = 0;
    int argc_out = -1;
    wchar_t arg0[] = L"pyrs66";
    wchar_t *argv_values[] = {arg0};
    PySys_SetArgv(1, argv_values);
    Py_GetArgcArgv(&argc_out, &argv_out);
    int argc_argv_ok = (
        argc_out == 1 &&
        argv_out != 0 &&
        argv_out[0] != 0 &&
        wcscmp(argv_out[0], arg0) == 0
    ) ? 1 : 0;

    size_t wlen = 0;
    wchar_t *decoded = Py_DecodeLocale("alpha", &wlen);
    size_t error_pos = 0;
    char *encoded = decoded ? Py_EncodeLocale(decoded, &error_pos) : 0;
    int locale_ok = (
        decoded != 0 &&
        encoded != 0 &&
        wlen == 5 &&
        strcmp(encoded, "alpha") == 0 &&
        error_pos == (size_t)-1
    ) ? 1 : 0;

    int prefix_exec_full_ok = (
        Py_GetPrefix() != 0 &&
        Py_GetExecPrefix() != 0 &&
        Py_GetProgramFullPath() != 0
    ) ? 1 : 0;
...
static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe path config + locale ABI APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch66_probe",
    "cpython api batch66 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch66_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch66 extension should build");

    run_import_snippet(
        &bin,
        &temp_root,
        "import cpython_api_batch66_probe as m\nres = m.run()\nassert res == (1, 1, 1, 1, 1, 1), res",
    )
    .expect("cpython api batch66 extension import should succeed");
...
#[test]
fn cpython_compat_lifecycle_and_pack_abi_batch67_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch67 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch67 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch67");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch67_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
#include <stdint.h>

static int atexit_hits = 0;
...
run(PyObject *self, PyObject *args) {
    ...
    int init_state_ok = (Py_IsFinalizing() == 0) ? 1 : 0;
    Py_InitializeEx(0);

    void *interp = Py_NewInterpreter();
    int interp_ok = (interp != 0) ? 1 : 0;

    uint32_t packed_full = Py_PACK_FULL_VERSION(3, 14, 2, 0xA, 5);
    uint32_t packed_short = Py_PACK_VERSION(3, 14);
    int pack_ok = (
        packed_full == 0x030E02A5u &&
        packed_short == 0x030E0000u
    ) ? 1 : 0;

    int atexit_ok = (Py_AtExit(on_exit) == 0) ? 1 : 0;

    int finalize_rc = Py_FinalizeEx();
    int finalize_ok = (
        finalize_rc == 0 &&
        atexit_hits == 1 &&
        Py_IsFinalizing() == 0
    ...
    Py_Finalize();
    int finalize_no_crash_ok = (Py_IsFinalizing() == 0) ? 1 : 0;
    ...
    return Py_BuildValue(
        ...
        atexit_ok,
        finalize_ok,
        finalize_no_crash_ok);
}

static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe lifecycle + pack ABI APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch67_probe",
    "cpython api batch67 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch67_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch67 extension should build");

    run_import_snippet(
        &bin,
        &temp_root,
        "import cpython_api_batch67_probe as m\nres = m.run()\nassert res == (1, 1, 1, 1, 1, 1), res",
    )
    .expect("cpython api batch67 extension import should succeed");
...
#[test]
fn cpython_compat_type_cache_coherence_abi_batch71_apis_work() {
    let Some(bin) = pyrs_bin() else {
        eprintln!("skipping cpython api batch71 smoke (pyrs binary not found)");
        return;
    };
    if !has_c_compiler() {
        eprintln!("skipping cpython api batch71 smoke (cc not available)");
        return;
    }

    let temp_root = unique_temp_dir("ext_smoke_cpython_api_batch71");
    fs::create_dir_all(&temp_root).expect("temp dir should be created");

    let source_path = temp_root.join("cpython_api_batch71_probe.c");
    fs::write(
        &source_path,
        r#"#include "pyrs_cpython_compat.h"
...
run(PyObject *self, PyObject *args) {
    ...
    unsigned int clear_tag_1 = PyType_ClearCache();
    unsigned int clear_tag_2 = PyType_ClearCache();
    int clear_tag_ok = (clear_tag_1 != 0 && clear_tag_2 != 0 && clear_tag_1 != clear_tag_2) ? 1 : 0;
    ...
    PyObject *cls = globals ? PyDict_GetItemString(globals, "C") : 0;
    PyObject *instance = globals ? PyDict_GetItemString(globals, "c") : 0;
    PyObject *replacement = globals ? PyDict_GetItemString(globals, "replacement") : 0;
    Py_XINCREF(cls);
    Py_XINCREF(instance);
    Py_XINCREF(replacement);

    int setup_ok = (cls && instance && replacement) ? 1 : 0;
    PyObject *baseline = setup_ok ? PyObject_CallMethod(instance, "f", 0) : 0;
    int baseline_ok = (baseline && PyLong_AsLong(baseline) == 1) ? 1 : 0;
    Py_XDECREF(baseline);

    int dict_write_ok = 0;
    if (setup_ok) {
        dict_write_ok = (PyObject_SetAttrString(cls, "f", replacement) == 0) ? 1 : 0;
        if (!dict_write_ok && PyErr_Occurred()) {
            PyErr_Clear();
        }
    }
    ...
    PyObject *after = setup_ok ? PyObject_CallMethod(instance, "f", 0) : 0;
    int modified_ok = (after && PyLong_AsLong(after) == 2) ? 1 : 0;
    Py_XDECREF(after);

    unsigned int clear_tag_3 = PyType_ClearCache();
    int clear_after_modified_ok = (clear_tag_3 != 0 && clear_tag_3 != clear_tag_2) ? 1 : 0;
    ...
    return Py_BuildValue(
        "(iiiii)",
        clear_tag_ok,
        setup_ok && baseline_ok,
        dict_write_ok,
        modified_ok,
        clear_after_modified_ok
    );
}

static PyMethodDef module_methods[] = {
    {"run", run, METH_NOARGS, "probe type cache coherence APIs"},
    {0, 0, 0, 0}
};

static struct PyModuleDef module_def = {
    PyModuleDef_HEAD_INIT,
    "cpython_api_batch71_probe",
    "cpython api batch71 probe module",
    -1,
    0,
    0,
    0,
...
PyMODINIT_FUNC
PyInit_cpython_api_batch71_probe(void) {
    PyObject *module = PyModule_Create(&module_def);
    if (!module) {
        return 0;
    }
...
"#,
    ...
    compile_shared_extension_with_cpython_compat(&source_path, &library_path)
        .expect("cpython api batch71 extension should build");

    run_import_snippet(
        &bin,
        &temp_root,
        "import cpython_api_batch71_probe as m\nres = m.run()\nassert res == (1, 1, 1, 1, 1), res",
    )
    .expect("cpython api batch71 extension import should succeed");
