...
#[test]
fn builtin_slot_wrapper_reprs_match_cpython_shape() {
    let source = r#"ok = (
    repr(type(list.__repr__)) == "<class 'wrapper_descriptor'>"
    ...
    and repr((1).__add__).startswith("<method-wrapper '__add__' of int object at 0x")
    and repr(bool.__add__) == "<slot wrapper '__add__' of 'int' objects>"
    and repr(True.__add__).startswith("<method-wrapper '__add__' of bool object at 0x")
    and repr(bool.__lt__) == "<slot wrapper '__lt__' of 'int' objects>"
    and repr(True.__lt__).startswith("<method-wrapper '__lt__' of bool object at 0x")
)
"#;
    let module = parser::parse_module(source).expect("parse should succeed");
    let code = compiler::compile_module(&module).expect("compile should succeed");
...
#[test]
fn abc_import_prefers_cpython_pure_module_when_lib_path_is_added() {
    let Some(lib_path) = cpython_lib_path() else {
        eprintln!("skipping pure-abc import preference test (CPython Lib path not available)");
        return;
    };
    let handle = std::thread::Builder::new()
        .name("abc-import-preference".to_string())
        .stack_size(32 * 1024 * 1024)
        .spawn(move || {
            let source = r#"import abc
origin = getattr(abc, '__file__', '')
norm = origin.replace("\\", "/")
class Proto(abc.ABC):
    @abc.abstractmethod
    def run(self):
...
ok = (
    norm.endswith('/abc.py')
    and ('run' in Proto.__abstractmethods__)
    and hasattr(abc, 'update_abstractmethods')
)
"#;
            let module = parser::parse_module(source).expect("parse should succeed");
            let code = compiler::compile_module(&module).expect("compile should succeed");
        ...
        .expect("spawn abc import preference thread");
    handle
        .join()
        .expect("abc import preference thread should complete");
}
...
#[test]
fn _io_module_exports_text_encoding_helper() {
    let source = r#"import _io
ok = hasattr(_io, 'text_encoding') and (_io.text_encoding(None) == 'utf-8')
"#;
    let module = parser::parse_module(source).expect("parse should succeed");
    let code = compiler::compile_module(&module).expect("compile should succeed");
    let mut vm = Vm::new();
...
#[test]
fn io_import_prefers_cpython_pure_module_when_lib_path_is_added() {
    let Some(lib_path) = cpython_lib_path() else {
        eprintln!("skipping pure-io import preference test (CPython Lib path not available)");
        return;
    };
    let handle = std::thread::Builder::new()
        .name("io-import-preference".to_string())
        .stack_size(32 * 1024 * 1024)
        .spawn(move || {
            let source = r#"import io
origin = getattr(io, '__file__', '')
norm = origin.replace("\\", "/")
ok = (
    norm.endswith('/io.py')
    and ('/shims/' not in norm)
    and hasattr(io, 'TextIOWrapper')
    and (io.text_encoding(None) == 'utf-8')
)
"#;
            let module = parser::parse_module(source).expect("parse should succeed");
            let code = compiler::compile_module(&module).expect("compile should succeed");
        ...
        .expect("spawn io import preference thread");
    handle
        .join()
        .expect("io import preference thread should complete");
}
...
#[test]
fn importlib_meta_path_exec_module_creates_and_registers_default_module() {
    let Some(lib_path) = cpython_lib_path() else {
        return;
    };
    run_with_large_stack("vm-importlib-meta-path-exec-default-module", move || {
        let source = "import builtins\nimport sys\nimport importlib.machinery as machinery\nevents = []\nclass Finder:\n    def find_spec(self, fullname, path=None, target=None):\n        if fullname == 'meta_exec_none_demo':\n            return machinery.ModuleSpec(fullname, self)\n        return None\n    def create_module(self, spec):\n        events.append('create')\n        return None\n    def exec_module(self, module):\n        events.append('exec')\n        module.loaded = True\nfinder = Finder()\nold_meta = sys.meta_path\nsys.meta_path = [finder]\ntry:\n    module = builtins.__import__('meta_exec_none_demo')\nfinally:\n    sys.meta_path = old_meta\nok = (events == ['create', 'exec'] and module.__name__ == 'meta_exec_none_demo' and module.__loader__ is finder and module.__spec__.loader is finder and module.loaded and sys.modules['meta_exec_none_demo'] is module)\n";
        let module = parser::parse_module(source).expect("parse should succeed");
        let code = compiler::compile_module(&module).expect("compile should succeed");
        let mut vm = Vm::new();
        vm.add_module_path(lib_path);
...
#[test]
fn import_uses_explicit_globals_for_relative_package_resolution() {
    let Some(lib_path) = cpython_lib_path() else {
        return;
    };
    let source = "import builtins\nimport sys\nimport importlib.machinery as machinery\nclass Finder:\n    def find_spec(self, fullname, path=None, target=None):\n        if fullname == 'pkg':\n            spec = machinery.ModuleSpec(fullname, self, is_package=True)\n            spec.submodule_search_locations = []\n            return spec\n        if fullname == 'pkg.fake':\n            return machinery.ModuleSpec(fullname, self)\n        return None\n    def create_module(self, spec):\n        return None\n    def exec_module(self, module):\n        if module.__name__ == 'pkg':\n            module.__path__ = []\nfinder = Finder()\nold_meta = sys.meta_path\nsys.meta_path = [finder]\ntry:\n    builtins.__import__('pkg.fake')\n    module = builtins.__import__('', globals={'__name__': 'pkg.fake', '__path__': [], '__package__': 'pkg.fake'}, fromlist=['attr'], level=2)\nfinally:\n    sys.meta_path = old_meta\nok = (module.__name__ == 'pkg' and sys.modules['pkg'] is module)\n";
    let module = parser::parse_module(source).expect("parse should succeed");
    let code = compiler::compile_module(&module).expect("compile should succeed");
    let mut vm = Vm::new();
    vm.add_module_path(lib_path);
...
#[test]
fn thread_module_exports_joinable_thread_api_surface() {
    let source = "import _thread\nh = _thread.start_joinable_thread(lambda: None)\nh2 = _thread._make_thread_handle(_thread.get_ident())\nready = h.is_done()\nh._set_done()\nok = (\n    hasattr(_thread, 'start_joinable_thread')\n    and callable(_thread.daemon_threads_allowed)\n    and hasattr(_thread, '_shutdown')\n    and hasattr(_thread, '_ThreadHandle')\n    and hasattr(_thread, 'LockType')\n    and isinstance(h, _thread._ThreadHandle)\n    and isinstance(h2, _thread._ThreadHandle)\n    and isinstance(h.ident, int)\n    and isinstance(ready, bool)\n    and h.is_done() is True\n)\n";
    let module = parser::parse_module(source).expect("parse should succeed");
    let code = compiler::compile_module(&module).expect("compile should succeed");
    let mut vm = Vm::new();
    vm.execute(&code).expect("execution should succeed");
...
#[test]
fn threading_pure_module_can_import_with_cpython_lib_path() {
    let Some(lib_path) = cpython_lib_path() else {
        eprintln!("skipping pure-threading import probe (CPython Lib path not available)");
        return;
    };
    let handle = std::thread::Builder::new()
        .name("threading-pure-import".to_string())
        .stack_size(32 * 1024 * 1024)
        .spawn(move || {
            let source = r#"import sys
sys.modules.pop('threading', None)
import threading
origin = getattr(threading, '__file__', '')
norm = origin.replace("\\", "/")
h = threading._make_thread_handle(threading.get_ident())
ok = (
    norm.endswith('/threading.py')
    and ('/shims/' not in norm)
...
"#;
        ...
        .expect("spawn pure-threading import probe");
    handle
        .join()
