...
- Next decomposition slice target: move remaining CPython C-API entrypoint clusters still in `vm_extensions.rs` (unicode + exception/file/traceback utility clusters) into focused `vm_extensions/*_api.rs` modules.
