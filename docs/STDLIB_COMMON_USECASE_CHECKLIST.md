...
| `sqlite3` | P0 | GREEN | in-memory connect/execute/fetch/close | `_sqlite3` now includes descriptor-backed connection attrs (`isolation_level`/`in_transaction`/`total_changes`), SQL-length DataError precheck, row/text-factory plumbing, and `_sqlite3.Row` baseline behavior; remaining DB-API long-tail (transactions/autocommit/types/factory edges) and one current failfast blocker (`test_connection_signature`) depend on broader object-model `__repr__`/`__str__` parity |
