...
| `sqlite3` | P0 | GREEN | in-memory connect/execute/fetch/close | `_sqlite3` now includes descriptor-backed connection attrs (`isolation_level`/`autocommit`/`in_transaction`/`total_changes`), SQL-length DataError precheck, row/text-factory plumbing, `_sqlite3.Row` equality + Sequence-ABC parity, real `create_function()` callback registration, callable `factory=`/path-like database handling, raw bytes/bytearray path handoff to sqlite without lossy UTF-8 replacement, native `check_same_thread` affinity enforcement for connection+cursor methods, `set_trace_callback()` statement delivery for VM-executed SQL, and `Connection.iterdump()` dispatch through CPython `Lib/sqlite3/dump.py`; strict stdlib suite now includes `test/test_sqlite3/test_dbapi.py`, `test/test_sqlite3/test_dump.py`, and `test/test_sqlite3/test_transactions.py` and is green; remaining DB-API long-tail frontier is type/runtime depth |
