...
| `sqlite3` | P0 | GREEN | in-memory connect/execute/fetch/close | `_sqlite3` baseline is landed (`connect`/cursor/query path, connection/cursor shortcuts, limit/config APIs, exported constants, and `Connection.blobopen`/`Blob` baseline methods); DB-API long-tail behavior (transactions/factories/types/isolation-level descriptors/reinit parity) remains |
