...
| Header surface + versioned symbol manifest | IN PROGRESS | runtime/ffi | `include/pyrs_capi.h`, `docs/EXTENSION_CAPI_V1.md` | v1 includes module-global setters/getters/import, callable registration, init-scoped object handles/refcount ops, object type/introspection getters, and error-state hooks. |
| `PyObject`/refcount ownership APIs | IN PROGRESS | runtime/ffi | `tests/extension_smoke.rs::dynamic_extension_can_set_module_values_via_object_handles`, `tests/extension_smoke.rs::dynamic_extension_can_call_python_callable_handles`, `tests/extension_smoke.rs::dynamic_extension_can_get_set_and_del_object_attributes`, `tests/extension_smoke.rs::dynamic_extension_can_check_isinstance_and_issubclass` | v1 handle model supports init-scoped object creation (`none`/`bool`/`int`/`float`/`bytes`/`tuple`/`list`/`dict`/`str`), typed getters, sequence+dict access, list mutation (`append`/indexed set), dict mutation (`set`/`get`/`contains`/`del`), object attribute get/set/del/has by handle+name, type relation checks (`object_is_instance`/`object_is_subclass`), handle-based callable invocation (`object_call`), and `incref`/`decref`; full CPython object surface remains open. |
