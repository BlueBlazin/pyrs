...
| Capsule/callback interop primitives | IN PROGRESS | runtime/ffi | `tests/extension_smoke.rs::dynamic_extension_can_use_capsule_apis` | Baseline capsule creation/name/pointer/context APIs are implemented in v1 handle space and support per-capsule destructors via `capsule_set_destructor`; import-level integration remains open. |
