...
| Extension module state lifecycle hooks | DONE | VM/extensions | `tests/extension_smoke.rs::dynamic_extension_can_manage_module_state_lifecycle` | `module_set_state`/`module_get_state`/`module_set_finalize` are wired with replacement/clear/churn/teardown finalize+free semantics. |
