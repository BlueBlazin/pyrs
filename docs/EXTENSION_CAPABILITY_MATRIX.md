...
| Surface | Status | Owner | Evidence | Notes |
|---|---|---|---|---|
| Buffer protocol producer/consumer parity | IN PROGRESS | runtime/object-model | `tests/extension_smoke.rs::dynamic_extension_can_use_buffer_apis` | Baseline pointer/len/readonly acquisition + release is implemented for `bytes`/`bytearray`/`memoryview`; full multi-dimensional/export-lifetime parity remains open. |
| Capsule/callback interop primitives | IN PROGRESS | runtime/ffi | `tests/extension_smoke.rs::dynamic_extension_can_use_capsule_apis` | Baseline capsule creation/name/pointer APIs are implemented in v1 handle space; destructor/context/import-level integration remains open. |
| ABI capability introspection API | IN PROGRESS | runtime/ffi | `tests/extension_smoke.rs::dynamic_extension_can_query_capabilities` | Baseline `api_has_capability(...)` probe now includes callable registration, module attr mutation (`module_set_attr`/`module_del_attr`/`module_has_attr`), generic item mutation (`object_set_item`/`object_del_item`), membership + dict-view helpers (`object_contains`/`object_dict_keys`/`object_dict_items`), buffer helpers (`object_get_buffer`/`object_release_buffer`), capsule helpers (`capsule_new`/`capsule_get_pointer`/`capsule_get_name`), dict/list handle mutation, and legacy v1 surfaces; richer capability taxonomy/versioning is still open. |
