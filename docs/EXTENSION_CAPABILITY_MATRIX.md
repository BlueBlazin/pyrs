...
|---|---|---|---|---|
| Extension smoke gate (compiled native fixture + `hello_ext`) | DONE | VM/extensions | `tests/extension_smoke.rs` + CI `Extension smoke lane` | CI covers manifest-only, compiled-manifest, direct shared-object, tagged-filename, mixed-surface cross-API fixture, invalid-handle resilience fixture, and error-path fixtures. |
