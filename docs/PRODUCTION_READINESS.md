...
- [ ] P0: Strict standalone `test_csv` unittest execution still fails under `unittest.runner`; prior `_csv` `StopIteration` propagation, `os.path`/`_io` discovery blockers (`relpath`/`isabs`, `TextIOWrapper` wrapping/readlines), and namedtuple tuple-like semantics (`_make`, iterable/len/indexing) are now closed, but remaining failures still include broader unittest/io/re/parity gaps. The strict harness currently isolates `test/test_csv.py` in a subprocess with a timeout guard while this blocker remains open; the prior runaway-growth failure mode from skipped `try/finally` cleanup in `import_helper.import_fresh_module` is now closed.
- [~] P0: Large `Lib/test` subset + CI gating (suite growth + allowlist reduction in progress).
- [ ] P0: Full CPython module-suite closure for `json`/`csv`/`pickle` stack (`test_json`, `test_csv`, `test_pickle`, `test_pickletools`, `test_copyreg`) with owned strict allowlist policy (`tests/cpython_allowlist_strict.txt`) burned down to zero (current strict blockers include remaining unittest-runner integration gaps, unresolved `pickle`/`io` protocol paths, and pickletools `super().__getattr__` execution-path parity). For faster local strict loops, allowlisted strict entries are skipped by default; set `PYRS_RUN_ALLOWLISTED_STRICT=1` to execute them.
- [~] P0: Pure-Python stdlib-first migration is active per `docs/STDLIB_MIGRATION_PLAN.md` (`json` CPython pure-import preference wiring is now available as explicit VM opt-in, while native fallback remains default until strict/runtime blockers are closed). Continue this pattern for remaining `csv`/`pickle`/`re` closure work.
...
- [x] P0: Prior strict pickle singleton/reducer and bytes API blockers are closed (`object.__reduce_ex__` now returns builtin singleton names for `Ellipsis`/`NotImplemented`; `bytes.join` is implemented for bytes-like iterables). Strict `test_pickle`/`test_pickletools` still time out under the current 120s/module strict harness watchdog.
