...
| `_io` behavioral parity needed by stdlib | `[~]` | Core mode/newline/validation landed; `io.FileIO`/`_io.FileIO.__init__` and `_io.StringIO`/`_io.BytesIO` close/context/open-state/readable/writable/seekable plus `read1`/`readlines`/`writelines`/`truncate`/`flush`/`isatty`, `getbuffer`/`detach`, `__getstate__`/`__setstate__`, and resize guards under active buffer exports are now wired (with stricter init/seek and `__index__`-style integer coercion), but full pure-`_pyio`/codec long-tail is still pending |
