...
| `_io` behavioral parity needed by stdlib | `[~]` | Core mode/newline/validation landed; `io.FileIO`/`_io.FileIO.__init__` and `_io.StringIO` method-surface gaps were closed, full stream long-tail still pending |
