...
- [ ] P0: Full CPython module-suite closure for `json`/`csv`/`pickle` stack (`test_json`, `test_csv`, `test_pickle`, `test_pickletools`, `test_copyreg`) with owned strict allowlist policy (`tests/cpython_allowlist_strict.txt`) burned down to zero (current strict blockers are concentrated in `test_pickle`/`test_pickletools` protocol/runtime parity and performance). For faster local strict loops, allowlisted strict entries are skipped by default; set `PYRS_RUN_ALLOWLISTED_STRICT=1` to execute them.
