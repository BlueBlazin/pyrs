...
- [~] P0: `_io.open` CPython file-object semantics (`TextIOWrapper.__init__` wrapping and `readlines()` are implemented; mode validation, binary/text argument compatibility checks, buffering guardrails, opener/FD closefd handling, legal newline-value validation, baseline text newline translation, public text-wrapper attrs (`encoding`/`errors`/`newlines`), and baseline `detach()` paths now track CPython `_io_open_impl` baselines. Full buffered-class behavior and wider stream-object parity are still required for complete `tempfile`/stdlib closure).
...
- [ ] P0: Full CPython module-suite closure for `json`/`csv`/`pickle` stack (`test_json`, `test_csv`, `test_pickle`, `test_pickletools`, `test_copyreg`) with active strict lane closure (`tests/cpython_suite_strict_stdlib.txt`; currently `test_json`, `test_csv`, `test_copyreg`, and `test_genericalias`) plus deferred pickle lane burn-down (`tests/cpython_suite_deferred_pickle.txt`).
