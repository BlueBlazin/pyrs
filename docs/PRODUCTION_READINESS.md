...
- [ ] P0: Strict standalone `test_csv` unittest execution still fails under `unittest.runner`; the prior `_csv` `StopIteration` propagation issue is fixed, and `os.path`/`_io` discovery blockers (`relpath`/`isabs`, `TextIOWrapper` wrapping/readlines) are now closed, but remaining failures still include broader unittest/io/re/parity gaps.
- [~] P0: Large `Lib/test` subset + CI gating (suite growth + allowlist reduction in progress).
- [ ] P0: Full CPython module-suite closure for `json`/`csv`/`pickle` stack (`test_json`, `test_csv`, `test_pickle`, `test_pickletools`, `test_copyreg`) with owned strict allowlist policy (`tests/cpython_allowlist_strict.txt`) burned down to zero (current strict blockers include `json.scanner.make_scanner` parity, `test_csv` unittest iteration path failures, pickle class-cleanup attribute parity, and pickletools `super().__getattr__` path parity).
