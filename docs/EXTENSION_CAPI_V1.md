...
- Buffer view structs:
  ...
  - `PyrsBufferInfoV1 { const uint8_t* data; uintptr_t len; int readonly; uintptr_t itemsize; uintptr_t ndim; intptr_t shape0; intptr_t stride0; const char* format; int contiguous; }`
  - `PyrsBufferInfoV2 { const uint8_t* data; uintptr_t len; int readonly; uintptr_t itemsize; uintptr_t ndim; const intptr_t* shape; const intptr_t* strides; const char* format; int contiguous; }`
...
- `module_set_finalize(void* module_ctx, PyrsModuleStateFinalizeV1 finalize_func)`
...
- `object_get_writable_buffer(void* module_ctx, PyrsObjectHandle object_handle, PyrsWritableBufferViewV1* out_view)`
- `object_get_buffer_info(void* module_ctx, PyrsObjectHandle object_handle, PyrsBufferInfoV1* out_info)`
- `object_get_buffer_info_v2(void* module_ctx, PyrsObjectHandle object_handle, PyrsBufferInfoV2* out_info)`
...
- `capsule_set_destructor(void* module_ctx, PyrsObjectHandle capsule_handle, PyrsCapsuleDestructorV1 destructor)`
- `capsule_get_destructor(void* module_ctx, PyrsObjectHandle capsule_handle)`
- `capsule_set_name(void* module_ctx, PyrsObjectHandle capsule_handle, const char* name)`
- `capsule_is_valid(void* module_ctx, PyrsObjectHandle capsule_handle, const char* name)` (`1`/`0` on success, `-1` on error)
- `capsule_export(void* module_ctx, PyrsObjectHandle capsule_handle)`
- `capsule_import(void* module_ctx, const char* name, int no_block)`
...
- buffer metadata v2 helper is available through `object_get_buffer_info_v2(...)` with pointer-based shape/stride metadata arrays (`shape[ndim]`, `strides[ndim]`); current runtime emits `ndim=1` buffers.
...
| buffer helpers (`get_buffer`/`get_writable_buffer`/`get_buffer_info`/`get_buffer_info_v2`/`release_buffer`) | `dynamic_extension_can_use_buffer_apis`, `dynamic_extension_can_use_writable_buffer_apis`, `dynamic_extension_can_read_buffer_info_metadata`, `dynamic_extension_buffer_info_marks_noncontiguous_slice_views`, `dynamic_extension_buffer_info_reflects_memoryview_cast_itemsize`, `dynamic_extension_buffer_pin_blocks_bytearray_resize_until_release`, `dynamic_extension_unreleased_buffer_pin_is_cleared_on_context_drop`, `dynamic_extension_buffer_api_handles_memoryview_slices_and_release`, `dynamic_extension_can_bridge_buffer_pointer_through_capsule` |
