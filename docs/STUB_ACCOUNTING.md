...
| `.pyc` exception-table execution | CPython 3.14 exception tables are currently unsupported in translated `.pyc` execution; runtime now explicitly falls back to source for such modules instead of executing with incorrect semantics. | Implement CPython exception-table-driven unwinding/handler dispatch for translated `.pyc` and retire source-fallback requirement for this gap. | pyc preference probes (`PYRS_IMPORT_PREFER_PYC=1`) plus targeted translated-`.pyc` regression tests covering `try`/`except`/`with`/`finally` flows | IN_PROGRESS | 13/14 |
