...
| `threading`/`signal`/`_thread`/`_warnings` | Foundations exist; behavior depth is incomplete. Current concrete blocker: `threading._register_atexit` (used by `concurrent.futures`). | Full in-scope behavioral parity with `concurrent.futures` common smoke green. | IN_PROGRESS | 13/16 |
