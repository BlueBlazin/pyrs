...
| `_sqlite3`/`sqlite3` | Baseline is broad; DB-API long-tail remains. Bytes/bytearray database arguments now flow to sqlite using raw bytes (no lossy UTF-8 replacement), path-like database arguments are accepted, connection+cursor thread-affinity checks are enforced when `check_same_thread=True`, and `Connection.iterdump()` is wired through CPython `Lib/sqlite3/dump.py::_iterdump`. | Close remaining DB-API long-tail (autocommit/type/threading-runtime edges and strict DB-API parity depth). | IN_PROGRESS | 13 |
