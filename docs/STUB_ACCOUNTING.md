...
| `importlib` / `importlib.util` | Core helper surface now includes non-`NoOp` `invalidate_caches` and baseline `spec_from_file_location`, but full CPython spec/loader object behavior and edge semantics are still partial | Full importlib helper parity required by stdlib and packaging workflows | Milestone 13 |
