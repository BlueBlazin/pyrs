...
| Builtin symbol surface (`builtins`) | Automated CPython-vs-pyrs gate is wired and currently green (`145/145` names; zero semantic-probe mismatches; allowlists empty) | Keep gate green with empty allowlists and no inventory/probe drift (`docs/BUILTIN_PARITY.md`) | 13 |
...
| `_sqlite3`/`sqlite3` | `_sqlite3` baseline expanded: connect/cursor/execute/fetch/close; connection shortcuts (`__call__`, `executemany`, `executescript`, context manager, `__del__`); descriptor-backed `Connection` attrs (`isolation_level`, `in_transaction`, `total_changes`); SQL-length DataError parity precheck; cursor `description`; row/text-factory plumbing; sqlite constant exports; `Connection.blobopen` + `_sqlite3.Blob`; and `_sqlite3.Row` baseline methods (`keys`, `__len__`, `__getitem__`, `__iter__`, `__eq__`). `create_function()` now registers real sqlite callbacks, callable `factory=` in `connect()` is wired, and sqlite callable signature rendering is aligned via `__text_signature__`. Top-stdlib common-usecase row remains green. | Remaining DB-API long-tail parity (autocommit/type adapters/blob edges + URI undecodable-path edge); current failfast frontier: `test_open_undecodable_uri` | 13 |
...
| Object-model protocol dispatch (`__contains__`/iterator fallback, slot edge parity) | Truthiness + baseline membership fallback order landed; long-tail slot/error edge parity still partial; print path now routes through `str()` instead of static formatting | Align remaining membership/slot edge semantics with CPython data model/tests | 13 |
