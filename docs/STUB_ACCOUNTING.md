...
| `decimal` / `_pylong` | `decimal.getcontext`/`setcontext`/`localcontext` now execute baseline non-`NoOp` context paths, but full decimal arithmetic/context semantics are still pending; `_pylong` remains stub-heavy | Replace remaining stubs with real semantics needed by stdlib/users | Milestone 13 |
