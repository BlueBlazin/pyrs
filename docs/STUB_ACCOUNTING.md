...
| `_sqlite3`/`sqlite3` | Baseline is broad; DB-API long-tail remains. Bytes/bytearray database arguments now flow to sqlite using raw bytes (no lossy UTF-8 replacement), and path-like database arguments are accepted. | Close remaining DB-API long-tail (autocommit/type/threading edges and strict DB-API parity depth). | IN_PROGRESS | 13 |
