...
| `operator` / `functools` | Core callable adapters now execute non-`NoOp` paths (`operator.itemgetter`/`attrgetter`/`methodcaller`, `functools.cmp_to_key` with `sorted`/`min`/`max` key ordering support); `functools.partial` now unwraps `staticmethod(...)` wrappers for `partialmethod(staticmethod(...), ...)` class-body compatibility; long-tail API/edge semantics still pending | Full module API and behavior parity with CPython tests | Milestone 13 |
