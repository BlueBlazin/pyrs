...
| `_io` | Core mode/newline/validation behavior implemented; `io.FileIO`/`_io.FileIO` constructor path landed; `_io.StringIO`/`_io.BytesIO` now cover close/context/open-state/readable/writable/seekable plus `read1`/`readlines`/`writelines`/`truncate`/`flush`/`isatty`, `getbuffer`/`detach`, `__getstate__`/`__setstate__`, buffer-export resize guards, and `__index__` integer-arg coercion; pure-`_pyio` compatibility still has long-tail blockers | Complete `_io` behavior required by strict stdlib and pure-stdlib execution (current next blocker: codec metadata gaps surfaced by `test_memoryio` / `CodecInfo._is_text_encoding`) | 13 |
