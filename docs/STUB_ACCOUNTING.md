...
| `_io` | Core mode/newline/validation behavior implemented; `io.FileIO`/`_io.FileIO` constructor path and `_io.StringIO` method surface now wired to native handlers; stream long-tail parity still incomplete | Complete `_io` behavior required by strict stdlib and pure-stdlib execution | 13 |
