...
|---|---|---|---|---|
| Extension ecosystem substrate (`.pyrs-ext` + shared-library loading) | Manifest-backed loader, direct shared-object loading (`.so/.dylib/.pyd`), compiled extension smoke coverage, tagged-filename resolution, C-API v1 slice (module setters + positional/keyword callable registration + init-scoped object handles/type getters + import-time error state), and `_sysconfigdata__*` extension-build var baseline (now with compile+import smoke) are landed. Remaining work: broaden C-ABI surface, lifecycle semantics (PEP 489), and ecosystem closure for NumPy/Pandas/Matplotlib/SciPy. | `docs/EXTENSION_CAPABILITY_MATRIX.md` marks required C runtime + lifecycle surfaces `DONE` with deterministic tests, and NumPy/Pandas/Matplotlib/SciPy bring-up gates are green in CI. | IN_PROGRESS | 15 |
