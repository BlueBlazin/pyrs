...
| `_sqlite3`/`sqlite3` | Baseline is broad; DB-API long-tail remains. Bytes/bytearray database arguments now flow to sqlite using raw bytes (no lossy UTF-8 replacement), path-like database arguments are accepted, connection+cursor thread-affinity checks are enforced when `check_same_thread=True`, `Connection.iterdump()` is wired through CPython `Lib/sqlite3/dump.py::_iterdump`, `autocommit` attribute transitions are implemented (`True`/`False`/`LEGACY_TRANSACTION_CONTROL`) with context-manager/commit/rollback semantics, `connect(factory=...)` now relays kwargs and preserves `Base Connection.__init__ not called.` parity for defective subclasses, and `Connection.cursor(factory=...)` now follows CPython callable/class dispatch (`factory` positional/keyword semantics, callable return-type validation, and native `Cursor.__init__` substrate). Strict stdlib suite runs `test/test_sqlite3/test_dbapi.py`, `test/test_sqlite3/test_dump.py`, and `test/test_sqlite3/test_transactions.py` green. | Close remaining DB-API long-tail (notably `test_factory` row-mapping/order semantics such as `dict(Row)` mapping conversion and row ordering error behavior, plus broader type/threading-runtime parity depth). | IN_PROGRESS | 13 |
