...
| Object-model protocol dispatch | Truthiness/membership baseline landed; long-tail slot/error semantics remain. `bytes.lstrip`/`bytes.strip` parity now covers `gzip.decompress` common path, memoryview typed scalar index/store format semantics are landed, and first-axis multidim memoryview slice/tolist shape+stride parity is landed; remaining memoryview multi-dimensional indexing/slice-assignment long-tail behavior is still open. | Align remaining protocol edge semantics with CPython data model/tests. | IN_PROGRESS | 13 |
