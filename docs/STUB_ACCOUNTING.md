...
| `_sqlite3`/`sqlite3` | `_sqlite3` baseline expanded: connect/cursor/execute/fetch/close; connection shortcuts (`__call__`, `executemany`, `executescript`, context manager, `__del__`); descriptor-backed `Connection` attrs (`isolation_level`, `in_transaction`, `total_changes`); SQL-length DataError parity precheck; cursor `description`; row/text-factory plumbing; sqlite constant exports; `Connection.blobopen` + `_sqlite3.Blob`; and `_sqlite3.Row` baseline methods (`keys`, `__len__`, `__getitem__`, `__iter__`, `__eq__`). inspect signature string rendering for sqlite callables is now aligned via `__text_signature__` consumption. Top-stdlib common-usecase row remains green. | Remaining DB-API long-tail parity (transactions/autocommit/factory/type adapters/blob edges); current failfast frontier: `test_in_transaction` | 13 |
