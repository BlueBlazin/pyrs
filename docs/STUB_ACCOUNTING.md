...
| `_sqlite3`/`sqlite3` | `_sqlite3` baseline landed (connect/cursor/execute/fetch/close, adapters/converters registries, core exception/type exports), sqlite constant exports now match CPython names, and `Connection.blobopen` + `_sqlite3.Blob` baseline methods (`read`/`write`/`seek`/`tell`/context/len/indexing) are wired; top-stdlib common-usecase row is green | Full DB-API semantic parity (transactions/factory callbacks/type adapters/edge errors), long-tail blob/index semantics, and expanded CPython suite coverage | 13 |
