...
| `dataclasses`/`typing`/`enum`/`contextvars` | Common paths are green; enum remains shim-backed by default while CPython `Lib/enum.py` path still fails on enum metaclass/base-resolution long-tail (`TypeError: ReprEnum subclasses must be mixed with a data type ...` in probe mode). `EnumType.__prepare__` namespace plumbing and `type.__new__` keyword/super baseline are landed. | Full in-scope semantics for modern pure-Python apps and removal of default enum-shim preference. | IN_PROGRESS | 13 |
