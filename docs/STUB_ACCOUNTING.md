...
| `_sqlite3`/`sqlite3` | `_sqlite3` baseline expanded: connect/cursor/execute/fetch/close; connection shortcuts (`__call__`, `executemany`, `executescript`, context manager); limit/config APIs (`getlimit`/`setlimit`, `getconfig`/`setconfig`); sqlite constant exports match CPython names; and `Connection.blobopen` + `_sqlite3.Blob` baseline methods (`read`/`write`/`seek`/`tell`/context/len/indexing/deletion errors) are wired; top-stdlib common-usecase row is green | Full DB-API semantic parity (transactions/factory callbacks/type adapters/isolation-level descriptors/`__init__` reinit semantics), remaining blob/memoryview long-tail edges, and expanded CPython suite coverage | 13 |
