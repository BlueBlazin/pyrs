...
| `pyexpat`/`xml` | `pyexpat` backend is missing; XML parser smoke currently fails in `xml.etree.ElementTree`. | `pyexpat` baseline lands for `ElementTree.fromstring()` core path and `xml` smoke turns green. | Extended probe (`perf/stdlib_compat_extended_latest.json`) shows `xml` smoke green + targeted XML parser regression test. | OPEN | 13 |
...
| `threading`/`signal`/`_thread`/`_warnings` | Foundations exist; behavior depth is incomplete. Current concrete blocker in `concurrent.futures` chain: iterator protocol gap (`itertools.count` lacking `__next__`). | Full in-scope behavioral parity with `concurrent.futures` common smoke green. | IN_PROGRESS | 13/16 |
...
| Compression extensions (`zlib`/`_bz2`/`_lzma`) | Native baselines now exist and close common import/one-shot flows for `bz2`/`lzma`; `gzip.decompress` still depends on pending `bytes.lstrip` object-model parity and streaming/filter-property semantics remain partial. | Extended probe + strict/curated suites for `gzip`/`bz2`/`lzma` are green without fallback shims, and no placeholder filter APIs remain. | IN_PROGRESS | 13 |
| Object-model protocol dispatch | Truthiness/membership baseline landed; long-tail slot/error semantics remain. Current concrete blocker: missing `bytes.lstrip` path used by `gzip.decompress`. | Align remaining protocol edge semantics with CPython data model/tests. | IN_PROGRESS | 13 |
