...
| `_sqlite3`/`sqlite3` | `_sqlite3` baseline expanded: connect/cursor/execute/fetch/close; connection shortcuts (`__call__`, `executemany`, `executescript`, context manager, `__del__`); descriptor-backed `Connection` attrs (`isolation_level`, `in_transaction`, `total_changes`); SQL-length DataError parity precheck; cursor `description`; row/text-factory plumbing; sqlite constant exports; `Connection.blobopen` + `_sqlite3.Blob`; and `_sqlite3.Row` baseline methods (`keys`, `__len__`, `__getitem__`, `__iter__`, `__eq__`). Top-stdlib common-usecase row remains green. | Remaining DB-API long-tail parity (transactions/autocommit/factory/type adapters/blob edges) plus `test_connection_signature` dependency on object-model special-method dispatch (`__repr__`/`__str__`) | 13 |
...
| Object-model protocol dispatch (`__contains__`/iterator fallback, slot edge parity) | Truthiness + baseline membership fallback order landed; long-tail slot/error edge parity still partial | Align remaining membership/slot edge semantics with CPython data model/tests, including honoring custom `__repr__`/`__str__` dispatch for user classes and inspect-visible objects | 13 |
