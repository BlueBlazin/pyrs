...
| `_io` | Core mode/newline/validation behavior implemented; `io.FileIO`/`_io.FileIO` constructor path landed; `_io.StringIO` and `_io.BytesIO` now expose close/context/open-state/readable/writable/seekable surfaces with stricter init/seek validation; stream long-tail parity still incomplete | Complete `_io` behavior required by strict stdlib and pure-stdlib execution | 13 |
