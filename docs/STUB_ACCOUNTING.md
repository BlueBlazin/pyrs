...
| `decimal`/`_pydecimal` | `decimal` is still a local bootstrap stub and currently shadows CPython `Lib/decimal.py`; common `Decimal(...)` constructor flow is partial (`class constructor takes no arguments`). | Migrate to CPython decimal path (or complete native `_decimal` surface) and pass common constructor/context smoke plus curated suites. | IN_PROGRESS | 13 |
