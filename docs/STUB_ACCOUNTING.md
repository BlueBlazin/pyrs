...
| `dataclasses`/`typing`/`enum`/`contextvars` | Common paths are green; enum remains shim-backed by default while CPython `Lib/enum.py` path still fails on class-namespace/metaclass long-tail (`EnumType.__prepare__` namespace handling and `type.__new__` keyword/super path). | Full in-scope semantics for modern pure-Python apps and removal of default enum-shim preference. | IN_PROGRESS | 13 |
