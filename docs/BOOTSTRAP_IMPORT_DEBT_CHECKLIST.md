...
- [x] `P1` Converted these helpers from eager snapshots to lazy iterator objects with CPython-shaped constructor behavior:
  - `accumulate`, `combinations`, `combinations_with_replacement`, `compress`, `dropwhile`, `filterfalse`, `islice`, `pairwise`, `permutations`, `product`, `repeat`, `starmap`, `takewhile`, `zip_longest`, `tee`, `batched`
