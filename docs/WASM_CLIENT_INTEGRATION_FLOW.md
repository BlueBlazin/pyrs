...
3. `wasm_worker_info()`
   - use `backend` + `supported` + `lifecycle_supported` + `execution_probe_enabled` + `execute_supported` for worker-runtime/readiness UI state.
...
With `WasmWorkerSession`, `info().state` also reflects the session-local
last lifecycle state after start/terminate/recycle calls.
