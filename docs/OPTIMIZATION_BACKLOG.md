...
- 2026-02-14 wave:
  ...
  - pyc-preference startup gap is now dominated by CPython exception-table execution: `PYRS_IMPORT_PREFER_PYC=1` fallback reasons are currently `unsupported CPython exception table (fallback to source required)` for modules such as `site`, `_sitebuiltins`, and `stat`.
