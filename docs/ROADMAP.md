...
- Active Milestone 13 blockers remain explicit: `_io.open` still lacks full CPython file-object semantics, and strict allowlist closure remains focused on `test_pickle`/`test_pickletools` timeout/perf + protocol/runtime gaps.
...
- Strict stdlib allowlist remains intentionally non-empty: current owned failures are narrowed to `test_pickle` and `test_pickletools` (`stdlib-strict-pickle` and `stdlib-strict-pickletools` categories in `tests/cpython_allowlist_strict.txt`), with prior `json.scanner.make_scanner`, `_csv` iterator propagation, `test_csv` `BadWriter` `OSError` handling, pickle `_class_cleanups`, protocol 4/5 frame corruption, dispatch-table fallback forwarding, bytes-alias identity, and list/dict chunk-op parity blockers closed.
...
- Strict stdlib lane remains P0-blocked on `test_pickle.py` and `test_pickletools.py` timeout/perf closure at current harness budgets; remaining work is concentrated in pickle runtime performance and long-tail protocol semantics tracked in Milestone 13 artifacts.
