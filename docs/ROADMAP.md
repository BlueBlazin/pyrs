...
- CPython harness breadth expansion now includes `test/test_set.py`, `test/test_tuple.py`, `test/test_slice.py`, `test/test_format.py`, and `test/test_configparser.py`; `test/test_list.py` remains tracked as blocked by class-statement inheritance hangs (`seq_tests`-style base classes), with ownership captured in `docs/STUB_ACCOUNTING.md`.
...
- Expanded CPython probe runs outside the curated harness now carry forward remaining Milestone 13 blockers tracked in `docs/STUB_ACCOUNTING.md` (class-statement inheritance hang for `seq_tests`-style bases and `_testinternalcapi.hamt`).
