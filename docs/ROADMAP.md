...
- Current curated language/import harness suites pass with an empty allowlist (`tests/cpython_allowlist.txt`), and the strict stdlib suite is active with an empty strict allowlist (`tests/cpython_allowlist_strict.txt`).
...
- Container internals advanced toward production-performance closure: dict now uses a dedicated open-addressing probing backend (CPython-style empty/dummy/occupied slots with perturb probing) and set/frozenset index maps use compact hash buckets (`One`/`Many`), reducing lookup/update overhead while preserving insertion-order/value semantics; remaining closure is load-factor/growth-policy tuning and long-tail hash/equality parity.
...
- Active Milestone 13 blockers remain explicit: `_io.open` still lacks full CPython file-object semantics, and full `pickle`/`pickletools` protocol/runtime closure remains open.
...
- Strict stdlib harness lane remains first-class: `tests/cpython_suite_strict_stdlib.txt` executes modules via `unittest` in `tests/cpython_harness.rs`; it is opt-in in local frequent loops (`PYRS_RUN_STRICT_STDLIB=1` or `PYRS_PARITY_STRICT=1`) and enabled by parity-gate runs.
- Strict allowlist is currently empty (`tests/cpython_allowlist_strict.txt`); deferred pickle strict closures are tracked separately in `tests/cpython_suite_deferred_pickle.txt`.
- Pure-stdlib migration execution started: `docs/STDLIB_MIGRATION_PLAN.md` now defines module ownership (`json`/`csv`/`pickle`/`re`) and VM import preference direction. `json` pure-module preference wiring is enabled by default (`Vm::enable_pure_json_preference` remains available for explicit control), with native fallback retained for stdlib-less environments.
...
- Pickle strict closure remains deferred from the active strict lane and is tracked in `tests/cpython_suite_deferred_pickle.txt`; remaining work is concentrated in pickle runtime performance and long-tail protocol semantics tracked in Milestone 13 artifacts.
...
- Strict harness observability/perf controls are expanded: `tests/cpython_harness.rs` now supports explicit subprocess binary selection (`PYRS_SUBPROCESS_BIN`, `PYRS_SUBPROCESS_BIN_MODE`) plus release-binary preference toggle (`PYRS_STRICT_PREFER_RELEASE`, default on), per-entry timing logs (`PYRS_STRICT_TIMING=1`), verbose unittest tracing (`PYRS_STRICT_UNITTEST_VERBOSITY`), and separate allowlisted timeout budget (`PYRS_STRICT_ALLOWLIST_TIMEOUT_SECS`, default 30s) so allowlisted-enabled loops stay bounded while preserving blocker visibility.
- Pickle runtime profiling and micro-optimization follow-up landed: optional `PYRS_PROFILE_PICKLE=1` summaries (`PYRS_PROFILE_PICKLE_EMIT_EVERY`) now report `object.__reduce_ex__` hot-path timings from VM-native pickle helpers, and copyreg helper callables are cached in-VM to avoid repeated import/getattr overhead; direct regression coverage now checks copyreg callable cache reuse in `src/vm/stdlib/pickle.rs`.
- `_io.open` parity follow-up landed using CPython `Modules/_io/_iomodule.c` (`_io_open_impl`) as reference: mode decoding/validation, binary-text argument guardrails (`encoding`/`errors`/`newline`), buffering validation (`buffering < -1`, unbuffered-text rejection), opener/FD handling (`closefd` restrictions, negative opener-fd rejection), and ValueError mapping regressions are now covered in `tests/vm.rs`.
- `_io.open` follow-up landed for newline semantics and stream layering: newline values are validated against CPython-legal forms, baseline universal-newline read translation and explicit write-translation paths are covered, and text wrappers now retain explicit `buffer`/`raw` links to raw file instances.
- `_io`/`tempfile` follow-up landed for strict stdlib parity: wrapped text file instances now expose baseline public `encoding`/`errors`/`newlines` attributes after rollover, and file objects now expose a baseline `detach()` path used by stdlib wrappers.
- `os`/startup parity follow-up landed for strict stdlib and `site` boot paths: `os.walk(..., onerror=...)` now invokes callbacks with `OSError`-family objects carrying `filename`/`errno`/`strerror`, `os.path.isjunction` baseline is available, and `sys.flags.no_site` is now wired from CLI `-S/--no-site` (with baseline `no_user_site`/`ignore_environment`/`isolated` fields and `sys.copyright` present).
- `_csv` validation follow-up landed for CPython-style value errors: empty `lineterminator` is accepted, delimiter/quotechar/escapechar conflict checks now emit CPython-style error families, and regression coverage now pins these contracts.
- Strict stdlib lane breadth expanded with `test/test_genericalias.py`; active strict non-pickle lane (`json`/`csv`/`copyreg`/`genericalias`) remains green with empty strict allowlist.
- `_json` accelerator-surface follow-up landed: local shim routing for `_json` is removed, and VM-native `_json` now exports `encode_basestring`, `encode_basestring_ascii`, and `make_encoder` alongside `scanstring`/`make_scanner` so CPython `Lib/json/encoder.py` C-accelerator import paths run without shim files; strict `test_json` lane remains green.
...
- `_sre` core bootstrap landed from CPython references (`Modules/_sre/sre.c`, `Lib/re/_compiler.py`): VM now provides `_sre.compile`, `_sre.template`, and ASCII/Unicode case helpers plus required constants. Super lookup and constructor flow were hardened for this path (`super(...).__new__` fallback + no double builtin-base initialization after successful custom `__new__`), with dedicated regressions in `tests/vm.rs` and `src/vm/stdlib/re.rs`.
- Module-namespace liveness parity follow-up landed for pure-stdlib imports: module-frame `globals()`/`locals()` now return live dict objects backed through frame sync (not snapshots), and module `__dict__` resolves through active frame namespace when present. This closes `Lib/re/_constants.py` `globals().update(...)` failures (`NameError: LITERAL`) and keeps strict/import harness lanes green.
