...
- Active Milestone 13 blockers remain explicit: `_io.open` still lacks full CPython file-object semantics, and strict allowlist closure remains focused on `test_pickle`/`test_pickletools` protocol/runtime gaps.
...
- Strict stdlib allowlist remains intentionally non-empty: current owned failures are narrowed to `test_pickle` and `test_pickletools` (`stdlib-strict-pickle` and `stdlib-strict-pickletools` categories in `tests/cpython_allowlist_strict.txt`); prior `json.scanner.make_scanner`, `_csv` iterator propagation, `test_csv` `BadWriter` `OSError` handling, and pickle `_class_cleanups` blockers are closed. Remaining gaps are concentrated in pickle protocol/runtime behavior and pickletools parity. These are tracked in `tests/cpython_allowlist_strict.txt` and `docs/STUB_ACCOUNTING.md`.
- Strict stdlib harness lane is now first-class: `tests/cpython_suite_strict_stdlib.txt` executes modules via `unittest` in `tests/cpython_harness.rs`, with owned allowlist tracking in `tests/cpython_allowlist_strict.txt`. `test/test_csv.py` passes in the isolated subprocess harness; the strict lane remains opt-in in local frequent loops (`PYRS_RUN_STRICT_STDLIB=1` or `PYRS_PARITY_STRICT=1`) and enabled by parity-gate runs, with allowlisted strict entries skipped by default for fast local iteration unless `PYRS_RUN_ALLOWLISTED_STRICT=1`.
- Pure-stdlib migration execution started: `docs/STDLIB_MIGRATION_PLAN.md` now defines module ownership (`json`/`csv`/`pickle`/`re`) and VM import preference direction. `json` pure-module preference wiring is available as an explicit VM opt-in (`Vm::enable_pure_json_preference`) and currently remains disabled by default while strict/runtime blockers are burned down (native JSON remains stable fallback).
...
- Strict stdlib lane remains P0-blocked on `test_pickle.py` and `test_pickletools.py` parity gaps (now failing fast under allowlisted strict runs, no runaway hangs observed); remaining closure work is concentrated in pickle runtime semantics/performance and pickletools identity-sensitive assertions.
...
- Strict harness observability/perf controls are expanded: `tests/cpython_harness.rs` now supports explicit subprocess binary selection (`PYRS_SUBPROCESS_BIN`, `PYRS_SUBPROCESS_BIN_MODE`), per-entry timing logs (`PYRS_STRICT_TIMING=1`), verbose unittest tracing (`PYRS_STRICT_UNITTEST_VERBOSITY`), and separate allowlisted timeout budget (`PYRS_STRICT_ALLOWLIST_TIMEOUT_SECS`, default 30s) so allowlisted-enabled loops stay bounded while preserving blocker visibility.
