...
- Strict stdlib allowlist remains intentionally non-empty: current owned failures are still `test_json` (notably `json.scanner.make_scanner` parity), `test_csv` (`unittest` runner path `iteration failed`), `test_pickle` (`_class_cleanups` class-attribute parity), and `test_pickletools` (`super().__getattr__` attribute path); these are tracked in `tests/cpython_allowlist_strict.txt` and `docs/STUB_ACCOUNTING.md`.
