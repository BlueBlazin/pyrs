...
| OM-005 | Special-method dispatch (`__repr__`/`__str__` and related hooks) | Baseline print path now routes through `str()`, but long-tail dispatch edges remain. | Close special-method dispatch parity required by stdlib/tests in scope. | targeted regressions + strict harness coverage | IN_PROGRESS |
