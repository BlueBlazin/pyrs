...
- [~] `pkgutil` / `importlib.resources` foundations (fallback shim workflows for stdlib-less environments: `pkgutil.get_data`, `importlib.resources.files/read_text/read_binary/open_*`; shim `read_*` paths now return payload bytes/text rather than file objects; full CPython parity still pending)
...
- [~] `os`, `pathlib`, `io` (`open`/`close`/`write`/`isatty`/`stat`/`lstat`/`rmdir`/`utime`/`scandir`/`getenv` and wait-status helpers now execute non-`NoOp` paths; `io.text_encoding` is implemented; `_io.open` now includes CPython-style mode/buffering/newline validation plus baseline newline translation behavior; full stream-object parity and broader module behavior remain pending)
...
- [~] `_csv` foundations (dialect registry helpers, `field_size_limit`, `Dialect` validation hook, reader `skipinitialspace`/`quotechar=None`/`escapechar`, and writer `quotechar=None`/`escapechar`/`quoting` (`QUOTE_MINIMAL`/`QUOTE_ALL`/`QUOTE_NONE`) are wired; full C-level parser/writer behavior parity remains pending)
...
- [~] `random` foundations (`seed`, `random`, `randrange`, `randint`, `getrandbits`, `choice`, `choices`, `shuffle`)
...
- [~] `json`, `re`, `datetime` (`json` now prefers CPython pure-module imports by default when stdlib paths are discoverable, while native fallback (`dumps` kwargs + UTF-8 `bytes`/`bytearray` `loads`) remains for stdlib-less environments pending strict-parity/runtime closure; full hook/edge parity still pending. `datetime` now exports baseline `date`/`timedelta` class symbols for stdlib import paths; full runtime parity still pending)
...
- [x] Current curated language/import harness suites are green with empty allowlist; latest expansion includes `test/test_set.py`, `test/test_list.py`, `test/test_tuple.py`, `test/test_slice.py`, `test/test_format.py`, `test/test_configparser.py`, `test/test_base64.py`, `test/test_binascii.py`, `test/test_bisect.py`, `test/test_copy.py`, `test/test_copyreg.py`, `test/test_csv.py`, `test/test_fnmatch.py`, `test/test_genericalias.py`, `test/test_heapq.py`, `test/test_pprint.py`, `test/test_reprlib.py`, `test/test_sched.py`, `test/test_statistics.py`, `test/test_textwrap.py`, `test/test_tokenize.py`, `test/test_json/__init__.py`, `test/test_dataclasses/__init__.py`, `test/test_enum.py`, `test/test_pkgutil.py`, and `test/test_importlib/resources/test_resource.py`.
...
- [~] Incremental `Lib/test` coverage expansion (broader suite growth and allowlist reduction ongoing; latest language-suite expansion adds `test_set`, `test_list`, `test_tuple`, `test_slice`, `test_format`, `test_configparser`, `test_base64`, `test_binascii`, `test_bisect`, `test_copy`, `test_copyreg`, `test_csv`, `test_fnmatch`, `test_genericalias`, `test_heapq`, `test_pprint`, `test_reprlib`, `test_sched`, `test_statistics`, `test_textwrap`, `test_tokenize`, `test_json/__init__`, `test_dataclasses/__init__`, and `test_enum`)
