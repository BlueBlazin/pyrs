...
- [~] `platform`, `binascii`, `atexit`, `collections` (`platform.win32_is_iot`, `binascii.crc32`, `atexit.register`/`unregister`/`_run_exitfuncs`/`_clear`, and `collections._count_elements` now execute non-`NoOp` paths; full module parity still pending)
...
- [~] `decimal`, `_thread`, `_warnings` (`decimal.getcontext`/`setcontext`/`localcontext`, `_thread.start_new_thread`, and `_warnings._acquire_lock`/`_release_lock` now execute non-`NoOp` baseline paths; full semantics still pending)
