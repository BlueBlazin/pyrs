...
- [~] `json`, `re`, `datetime` (`json` has CPython pure-module preference wiring as an explicit VM opt-in (`Vm::enable_pure_json_preference`), while native fallback (`dumps` kwargs + UTF-8 `bytes`/`bytearray` `loads`) remains default pending strict-parity/runtime closure; full hook/edge parity still pending. `datetime` now exports baseline `date`/`timedelta` class symbols for stdlib import paths; full runtime parity still pending)
