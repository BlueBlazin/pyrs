...
- `[~]` `_io` parity advanced (`io.FileIO` + `_io.FileIO.__init__`, `_io.StringIO`/`_io.BytesIO` close/context/open-state/readable/writable/seekable, `read1`/`readlines`/`writelines`/`truncate`/`flush`/`isatty`, `getbuffer`/`detach`, `__getstate__`/`__setstate__`, buffer-export resize guards, and incremental codec factory/state support); filtered pure-`_pyio` `test_memoryio` lane now green, with remaining long-tail still pending
