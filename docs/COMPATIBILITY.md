...
- [~] `operator`, `functools` (`operator.itemgetter`/`attrgetter`/`methodcaller` and `functools.cmp_to_key` now execute non-`NoOp` paths with `sorted`/`min`/`max` key interoperability; `functools.partial` unwraps `staticmethod(...)` wrappers for `partialmethod(staticmethod(...), ...)` class-body compatibility; long-tail API parity still pending)
- [~] `json`, `re`, `datetime`
...
- [~] `threading` foundations (`get_ident`, `current_thread`, `main_thread`, `active_count`, plus baseline class methods for `Thread`, `Event`, `Condition`, `Semaphore`, `BoundedSemaphore`, `Barrier`)
