...
- [~] `json`, `re`, `datetime` (`json` now supports baseline `dumps` kwargs (`sort_keys`, `separators`, `ensure_ascii`, `allow_nan`, `default`) and UTF-8 `bytes`/`bytearray` `loads`; full hook/edge parity still pending. `datetime` now exports baseline `date`/`timedelta` class symbols for stdlib import paths; full runtime parity still pending)
