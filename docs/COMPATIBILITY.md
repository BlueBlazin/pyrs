...
- [~] `os`, `pathlib`, `io` (`open`/`close`/`write`/`isatty`/`stat`/`lstat`/`rmdir`/`utime`/`scandir`/`getenv` and wait-status helpers now execute non-`NoOp` paths; `io.text_encoding` is implemented; full `_io.open` stream-object parity and broader module behavior remain pending)
