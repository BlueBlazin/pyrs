...
- [~] `operator`, `functools` (`operator.itemgetter`/`attrgetter`/`methodcaller` and `functools.cmp_to_key` now execute non-`NoOp` paths with `sorted`/`min`/`max` key interoperability; `functools.partial` unwraps `staticmethod(...)` and `classmethod(...)` wrappers for partial/partialmethod class-body compatibility; long-tail API parity still pending)
- [~] `json`, `re`, `datetime`
